...
\subsubsection{Kalpuris xxx-System}
...
\node at (3,0) (prep) {Vorverarbeitung};
\node at (6.2,0) (onset) {Onset-Detektion};
\node at (9.5,0) (rasta) {RASTA-Filtering};
\node at (12.9,0) (multi) {Multipitch-Schätzung};
% Neue Zeile (links unten)
\node at (0,-2) (voice) {Stimmenanzahl-Schätzung};
\node at (3.5,-2) (notes) {Note Formation};
\node at (6.5,-2) (output) {Transkription};
% Pfeile obere Zeile
\draw[->] (input) -- (prep);
\draw[->] (prep) -- (onset);
\draw[->] (onset) -- (rasta);
\draw[->] (rasta) -- (multi);
% Pfeile untere Zeile
\draw[->] (multi.south) -- ++(0,-0.3) -- ++(-12.9,0) |- (voice);
\draw[->] (voice) -- (notes);
\draw[->] (notes) -- (output);
\end{tikzpicture}
...
% RASTA-basiertes Verfahren für unharmonische Instrumente erklären
% Modul zur Stimmanzahl erkennung erklären
